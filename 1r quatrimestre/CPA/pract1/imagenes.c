...
int Filtro(const int pasos, const int radio, 
			const struct pixel **ppsImagenOrg, struct pixel **ppsImagenDst,
	...
	for (i = -radio; i <= radio; i++)
  	...
  	// Vegades que s'aplica el filtro
	#pragma omp parallel for private(pas, resultado, tot)
  	for (pas = 0; pas < pasos; pas++) {
		// Sub-grup, i és pixel central (y)
		for (i = 0; i < n; i++) {
			// Sub-grup, j és pixel central (x)
			for (j = 0; j < m; j++) {
				// Components del struct rgb 3 resultant
				resultado.r = 0;
				resultado.g = 0;
				resultado.b = 0;
				...
				// Cada pixel en el sub-grup (y)
 				for (k = max(0, i - radio); k <= min(n - 1, i + radio); k++) {
					// Cada pixel en el sub-grup (x)
					for (l = max(0, j - radio); l <= min(m - 1, j + radio); l++) {
						// Multiplicador del efecte en cada pixel del sub-grup
						// (disminueix en base a la distància al pixel central)
						v = ppdBloque[k - i + radio][l - j + radio];

						// ppsImagenOrg és array<array<struct(int 3 rgb)>> de imagte inicial
				...
				// Divideix per a fer mitjana aritmètica
				resultado.r /= tot;
				resultado.g /= tot;
				resultado.b /= tot;

				// Es guarda en imatge final
...
int main()
{
	...
	// El for era per a comprovar la eficiència de les diferents opcions
	// que em pareixien plausibles
